...
class ROTPooling(nn.Module):
    def __init__(self, dim: int = 512, a0: float = None, a1: float = None, a2: float = None, a3: float = None,
...
class UOTPooling(nn.Module):
    def __init__(self, dim: int = 512, output_dim: int = 2048, a1: float = None, a2: float = None, a3: float = None, rho: float = None,
...
#         self.linear_r1 = nn.Linear(in_features=self.dim, out_features=self.output_dim, bias=False)
#         self.linear_r2 = nn.Linear(in_features=self.output_dim, out_features=1, bias=False)
        self.tanh = nn.Tanh()
...
#         q0, _ = to_dense_batch(q0, batch)  # (B, Nmax, 1)
#         p0 = (torch.ones_like(x[:, 0, :]) / self.dim).unsqueeze(1)  # (B, 1, D)
        B, C, H, W = x.shape # (128, 2048, 7, 7)
        x = x.view(B, C, H*W).permute(0, 2, 1) # (128, 2048, 49)
        q0 = torch.ones_like(x[:,:,0]).unsqueeze(2) # (128, 49, 1)
        p0 = (torch.ones_like(x[:,0,:]) / self.dim).unsqueeze(1) # (128, 1, 2048)
        ...
        if a3.shape[0] == 1:
            a3 = a3.repeat(self.num)
        trans = self.uot(x, p0, q0, a1, a2, a3, rho)  # (B, Nmax, D)
        frot = self.dim * x * trans  # (B, Nmax, D)
...
class UOTLoss(nn.Module):
    def __init__(self, dim: int = 512, output_dim: int = 2048, a1: float = None, a2: float = None, a3: float = None, rho: float = None,
        ...
        self.sigmoid = nn.Sigmoid()
        self.softmax1 = nn.Softmax(dim=1)
        self.uot = UOT(num=self.num, eps=self.eps, f_method='loss')
    ...
    def forward(self, x):
        ...
        :return:
            a pooling result with size (K, D)
        """
        q0 = torch.ones_like(x)
        p0 = (torch.ones_like(x) / self.dim)
        ...
        if a3.shape[0] == 1:
            a3 = a3.repeat(self.num)
        trans = self.uot(x, p0, q0, a1, a2, a3, rho)  # (B, Nmax, D)
        return self.dim * x * trans  # (B, Nmax, D)
