...
def uot_badmm(x: torch.Tensor, p0: torch.Tensor, q0: torch.Tensor,
    ...
    for k in range(num):
        ...
        log_t = (log_eta - torch.logsumexp(y, dim=2, keepdim=True)) + y  # (B, N, D)
        # update logS
        y = (z + rho[n] * log_t) / (a1[n] + rho[n])  # (B, N, D)
        ymin, _ = torch.min(y, dim=1, keepdim=True)
        ymax, _ = torch.max(ymin - ymin + y, dim=1, keepdim=True)  # (B, 1, D)
        # (B, N, D)
        log_s = (log_mu - torch.log(torch.sum(torch.exp((y - ymax)), dim=1, keepdim=True)) - ymax) + y
        ...
        ymax, _ = torch.max(ymin - ymin + y, dim=1, keepdim=True)  # (B, 1, D)
        log_eta = (y - torch.log(
            torch.sum(torch.exp((y - ymax)), dim=1, keepdim=True)) - ymax)  # (B, N, 1)
        # update dual variables
        z1 = z1 + rho[n] * (torch.exp(log_mu) - torch.sum(s, dim=1, keepdim=True))  # (B, 1, D)
        z2 = z2 + rho[n] * (torch.exp(log_eta) - torch.sum(t, dim=2, keepdim=True))  # (B, N, 1)
    return torch.exp(log_t)

def uot_loss(x: torch.Tensor, p0: torch.Tensor, q0: torch.Tensor,
              a1: torch.Tensor, a2: torch.Tensor, a3: torch.Tensor, rho: torch.Tensor,
              num: int = 4, eps: float = 1e-8) -> torch.Tensor:
    """
    Solving regularized optimal transport via Bregman ADMM algorithm (entropic regularizer)
    ...
    for k in range(num):
        ...
        # update dual variables
        z1 = z1 + rho[n] * (torch.exp(log_mu) - torch.sum(s, dim=1, keepdim=True))  # (B, 1, D)
        z2 = z2 + rho[n] * (torch.exp(log_eta) - torch.sum(t, dim=1, keepdim=True))  # (B, N, 1)
    return torch.exp(log_t)
...
def uot_badmm2(x: torch.Tensor, p0: torch.Tensor, q0: torch.Tensor,
    ...
    for k in range(num):
        n = min([k, a1.shape[0] - 1])
        # update logP
        y = (x - a1[n] * torch.exp(log_s) - z) / rho[n] + log_s  # (B, N, D)
        log_t = (log_eta - torch.logsumexp(y, dim=2, keepdim=True)) + y  # (B, N, D)
        # update logS
        y = (z - a1[n] * torch.exp(log_t)) / rho[n] + log_t  # (B, N, D)
        ymin, _ = torch.min(y, dim=1, keepdim=True)
        ymax, _ = torch.max(ymin - ymin + y, dim=1, keepdim=True)  # (B, 1, D)
        # (B, N, D)
        log_s = (log_mu - torch.log(torch.sum(torch.exp((y - ymax)), dim=1, keepdim=True)) - ymax) + y
...
class UOT(nn.Module):
    ...
    def forward(self, x, p0, q0, a1, a2, a3, rho):
        """
        Solving regularized OT problem
        """
        if self.f_method == 'badmm-e':
            t = uot_badmm(x, p0, q0, a1, a2, a3, rho, self.num, self.eps)
        elif self.f_method == 'badmm-q':
            t = uot_badmm2(x, p0, q0, a1, a2, a3, rho,  self.num, self.eps)
        elif self.f_method == 'loss':
            t = uot_loss(x, p0, q0, a1, a2, a3, rho,  self.num, self.eps)
        else:
            t = uot_sinkhorn(x, p0, q0, a1, a2, a3, self.num, eps=self.eps)
        return t
